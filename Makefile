...
# ─── 构建时配置 ──────────────────────────────────────
# 数据源配置：通过 ldflags 注入 internal/config 包
API_BASE_URL  ?= ""
DATA_SOURCE   ?= ""
LDFLAGS       := $(if $(API_BASE_URL),-X 'github.com/user/lottery/internal/config.APIBaseURL=$(API_BASE_URL)') $(if $(DATA_SOURCE),-X 'github.com/user/lottery/internal/config.DataSource=$(DATA_SOURCE)')
GO_FLAGS      := $(if $(LDFLAGS),-ldflags="$(LDFLAGS)")
...
apk: check-gogio check-android $(ICON_PNG)  ## 构建 Android 调试 APK
	export PATH="$$PATH:$$HOME/go/bin" ANDROID_HOME="$(ANDROID_HOME)" ANDROID_NDK_HOME="$(ANDROID_NDK_HOME)" \
	  && gogio -target android -arch arm64 -appid "$(APP_ID)" -icon "$(ICON_PNG)" -minsdk 21 -ldflags="$(LDFLAGS)" -o "$(DEBUG_APK)" .
	@echo "✅ $(DEBUG_APK) 构建完成"
	@ls -lh "$(DEBUG_APK)"
...
release: check-gogio check-android check-key $(ICON_PNG)  ## 构建已签名的发布 APK
	...
	export PATH="$$PATH:$$HOME/go/bin" ANDROID_HOME="$(ANDROID_HOME)" ANDROID_NDK_HOME="$(ANDROID_NDK_HOME)" \
	  && gogio -target android -arch arm64 -appid "$(APP_ID)" -icon "$(ICON_PNG)" -ldflags="$(LDFLAGS)" \
